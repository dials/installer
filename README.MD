...
On Windows and Linux the installer also adds a **Launch DIALS environment**
menu entry, which opens a shell with DIALS already on `PATH`. There is no
equivalent menu entry on macOS; use `setpaths.sh` there.
